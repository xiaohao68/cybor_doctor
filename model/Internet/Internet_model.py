...
#VectorStoreRetriever 用于从向量数据库中检索相似的文档，一般用于表示变量是向量数据库的检索器类
...
class InternetModel(Modelbase):
    ...
    def build(self):
        ...
        html_loader = DirectoryLoader(
            ...
            #loader_cls 表示用于加载HTML文件的类，这里使用 UnstructuredHTMLLoader
            #silent_errors 表示在加载文件时遇到错误时是否静默处理，这里设置为 True
            #use_multithreading 表示是否使用多线程加载文件，这里设置为 True
            loader_cls=UnstructuredHTMLLoader, silent_errors=True, use_multithreading=True)
        html_docs = html_loader.load()
        
        #mhtml是微软的html文件，包含元数据和脚本信息，用于保存网页的完整状态，
        #与html文件不同的是，mhtml文件可以包含图片、视频等多媒体内容，内容更完整
        mhtml_loader = DirectoryLoader(self._data_path, glob="**/*.mhtml",
         loader_cls=MHTMLLoader, silent_errors=True, use_multithreading=True)
