...
from client.clientfactory import Clientfactory           # 导入客户端工厂

from qa.prompt_templates import get_question_parser_prompt  # 导入问题解析提示词模板
from qa.purpose_type import purpose_map                  # 导入意图映射字典
from qa.purpose_type import userPurposeType              # 导入意图类型枚举
...
def parse_question(question: str, image_url=None) -> userPurposeType:
    ...
    # 规则7：使用大模型进行意图分类（处理复杂情况）
    prompt = get_question_parser_prompt(question)           # 获取分类提示词
    response = Clientfactory().get_client().chat_with_ai(prompt)  # 调用LLM进行分类
