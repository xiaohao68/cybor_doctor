...
docx_prompt = f'''请你根据用户要求生成docx的详细内容，不要省略。
按这个JSON格式输出{__docx_schema}，只能返回JSON，且JSON不要用```包裹，不要返回markdown格式'''
...
def __build_messages(question: str, history: List[List | None]) -> List[Dict[str, str]]:
    ...
    messages.append({"role": "user", "content": docx_prompt})

    return messages

# 生成 docx 内容的函数
def build_docx_json(question: str,
